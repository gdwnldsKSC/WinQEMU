...
#else
	if (fx80_isl_fx80 (&a, &b)) {
		return float_relation_less;
	} else if (fx80_isequal_fx80 (&a, &b)) {
		return float_relation_equal;
	} else if (fx80_isg_fx80 (&a, &b)) {
		return float_relation_greater;
	} else {
		return float_relation_unordered;
}
