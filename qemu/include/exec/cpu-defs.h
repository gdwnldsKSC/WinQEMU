...
#endif
#else
/* MSVC modification: alignment is a prefix __declspec, not a suffix
   attribute.  Provide the alignment defaults (normally from config-target.h)
   and declare the target types with __declspec(align()). */
...
#if CPU_TLB_ENTRY_BITS == 5    /* dummy is 0 on 32-bit hosts (the struct is
       already 16 bytes = a power of two); MSVC rejects such a zero-size array
       (C2233), so only emit the padding on 64-bit hosts where it is nonzero. */
