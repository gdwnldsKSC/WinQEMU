                                                   ...
                                                   Error **errp)
{
    ...
    do {
        for (prop = DEVICE_CLASS(klass)->props; prop && prop->name; prop++) {
            ...
            info->type = g_strdup(prop->info->legacy_name ? prop->info->legacy_name : prop->info->name);
...
#if !defined(_MSC_VER) || !(defined(TARGET_I386) || defined(TARGET_PPC))
/* MSVC modification: MSVC has no weak symbols, so this default would
   collide with the strong definition provided by targets that implement
   query-cpu-definitions (i386 in target-i386/cpu.c, ppc in
   target-ppc/translate_init.c).  Keep it only for targets without one. */
